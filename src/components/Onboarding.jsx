...
export default function Onboarding({ onComplete }) {
  ...
  const handleCustomCitySubmit = () => {
    ...
    const updated = { ...answers, city: customCity.trim() };
    setAnswers(updated);
    advance(updated);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-950 via-slate-900 to-teal-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 relative z-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-emerald-500/20 backdrop-blur-sm mb-4 border border-emerald-500/30">
        ...
        <div className="mb-8 relative z-10">
          <div className="flex justify-between text-xs text-emerald-400/70 mb-2 font-medium">
            <span>Step {step + 1} of {steps.length}</span>
            <span>{Math.round(progress)}%</span>
          </div>
          <div className="h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
            <div className="h-full bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full transition-all duration-500 ease-out" style={{ width: `${progress}%` }} />
          </div>
        </div>
        ...
        <div key={step} className="relative z-10 animate-[fadeIn_0.3s_ease]">
          {!showCustomCityInput ? (
            <>
              <h2 className="text-xl font-bold text-white mb-6 leading-snug">{current.question}</h2>
              <div className="space-y-3">
                {current.options.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => handleSelect(opt.value)}
                    className="w-full text-left px-5 py-4 rounded-2xl border bg-white/5 border-white/10 text-white/90 hover:bg-white/10 hover:border-emerald-500/30 active:scale-[0.99] transition-all duration-200 flex items-center justify-between group"
                  >
            ...
            <div className="space-y-5">
              ...
              <button
                ...
                className="w-full py-4 rounded-2xl font-bold text-white tracking-wide bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 shadow-lg shadow-emerald-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Complete Onboarding 🚀
              </button>
              <button
        ...
        {step > 0 && !showCustomCityInput && (
          <button
            ...
            className="mt-6 text-sm font-medium text-emerald-400/60 hover:text-emerald-400 transition-colors mx-auto block"
          >
            ← Previous Question
          </button>
        )}
